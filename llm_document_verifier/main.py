...
def run_llm_to_get_json_from_contract(contract_file: gr.File, output_file_path: str) -> str:
    """Extract text from a DOCX file and retry if JSON loading fails."""
    contract2json_system_prompt: str = config["prompts"]["contract2json_system"]

    contract_text: str = Docx2txtLoader(contract_file.name).load()[0].page_content
    ...
    for attempt in range(config["openai"]["max_retries"]):
        ...
        try:
            ...
            logger.error("Failed to parse JSON on attempt %d: %s", attempt + 1, response_cleaned)
    else:
...
def parse_tasks_file(task_file: gr.File) -> list[tuple[int, str]]:
    ...
    return [
        (index, f"Task description: {row['Task Description']}, Amount: {row['Amount']}")
        for index, row in df.iterrows()
    ]

def llm_check_tasks(json_data: str, tasks: list[tuple[int, str]], progress: gr.Progress) -> list[dict[str, any]]:
    """Batch execute tasks, retrying failed JSON loads."""
    remaining_tasks: list[tuple[int, str]] = tasks
    tasks_decisions: list[dict[str, any]] = []
    processed_tasks: set[int] = set()  # To keep track of tasks that have been processed
    ...
    for attempt in range(config["openai"]["max_retries"]):
        ...
        for i, response in enumerate(responses):
            ...
            try:
                ...
                task_decision: dict[str, any] = {'Task Number': remaining_tasks[i][0],
                                                 'Task': remaining_tasks[i][1]}
                task_decision.update(response_data)
                tasks_decisions.append(task_decision)
                processed_tasks.add(remaining_tasks[i][0])  # Mark this task as processed

                progress(len(processed_tasks) / total_tasks)
            except json.JSONDecodeError:
                logger.warning("Failed to decode JSON response for task %d on attempt %d: %s",
    ...
    for task in remaining_tasks:
        if task[0] not in processed_tasks:  # Only add if the task hasn't been successfully processed
            logger.error("Max retries reached for task %d. No valid JSON response obtained.", task[0])
            tasks_decisions.append(
                {'Task Number': task[0],
                 'Task': task[1],
                 'Error': 'Failed to parse JSON after maximum retries'})

    tasks_decisions.sort(key=lambda x: x['Task Number'])
    return tasks_decisions

def handle_tasks(contract_file_path: str, tasks_file: gr.File, output_file_path: str, progress: gr.Progress) -> str:
...
def handle_files_upload(contract_file: gr.File, tasks_file: gr.File | None, progress=gr.Progress()) -> list[str | None]:
    ...
    if contract_file.name.endswith('.json'):
        ...
        output_json_contract_path: str = os.path.splitext(contract_file.name)[0] + '.json'
        run_llm_to_get_json_from_contract(contract_file, output_json_contract_path)

    output_csv_tasks_path: str | None = None

    if tasks_file is not None:
        output_csv_tasks_path = handle_tasks(
            output_json_contract_path, tasks_file, config["paths"]["output_file"], progress)
...
def main() -> None:
    ...
    interface = gr.Interface(
        fn=lambda contract_file, tasks_file=None: handle_files_upload(contract_file, tasks_file),
        inputs=["file", "file"],
        outputs=[
